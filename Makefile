...
aom := $(instdir)/lib/libaom.a
ass := $(instdir)/lib/libass.so
caca := $(instdir)/lib/libcaca.so
codec2 := $(instdir)/lib/libcodec2.so
fdk := $(instdir)/lib/libfdk-aac.a
flac := $(instdir)/bin/flac
fribidi := $(instdir)/bin/fribidi
gme := $(instdir)/lib/libgme.so
gsm := $(instdir)/lib/libgsm.a
jpeg := $(instdir)/lib/libopenjp2.so
ladspa := $(instdir)/include/ladspa.h
mad := $(instdir)/lib/libmad.a
mysofa := $(instdir)/lib/libmysofa.so
nasm := $(instdir)/bin/nasm
nvcodec := $(instdir)/lib/pkgconfig/ffnvcodec.pc
ogg := $(instdir)/lib/libogg.a
opencore := $(instdir)/lib/libopencore-amrnb.so
opus := $(instdir)/lib/libopus.a
rtmp := $(instdir)/lib/librtmp.so
rubberband := $(instdir)/lib/librubberband.so
speex := $(instdir)/lib/libspeex.so
theora := $(instdir)/lib/libtheora.a
va := $(instdir)/lib/libva.so
vamp := $(instdir)/lib/libvamp-hostsdk.la
vidstab := $(instdir)/lib/libvidstab.so
#vo := $(instdir)/
vorbis := $(instdir)/lib/libvorbis.a
vpx := $(instdir)/bin/vpxenc
wavpack := $(instdir)/lib/libwavpack.so
webp := $(instdir)/lib/libwebp.a
x264 := $(instdir)/bin/x264
x265 := $(instdir)/bin/x265
xvid := $(instdir)/lib/libxvidcore.so
zvbi := $(instdir)/lib/libzvbi.so
...
all := $(ffmpeg) $(a52) $(aom) $(ass) $(caca) $(codec2) $(fdk) $(flac) $(fribidi) $(gme) $(gsm) $(jpeg) $(ladspa) $(mad) $(mysofa) $(nasm) $(nvcodec) $(ogg) $(opencore) $(opus) $(rtmp) $(rubberband) $(speex) $(theora) $(va) $(vamp) $(vidstab) $(vorbis) $(vpx) $(wavpack) $(webp) $(x264) $(x265) $(xvid) $(zvbi) 
# Default build target
default: $(all)

# Define build dependencies
#$(vlc): vlc-2.2.8.tar.xz $(ffmpeg)
$(ffmpeg): ffmpeg-4.2.tar.bz2 $(a52) $(aom) $(ass) $(caca) $(codec2) $(fdk) $(flac) $(fribidi) $(gme) $(gsm) $(jpeg) $(ladspa) $(mad) $(mysofa) $(nasm) $(nvcodec) $(ogg) $(opencore) $(opus) $(rtmp) $(rubberband) $(speex) $(theora) $(va) $(vamp) $(vidstab) $(vorbis) $(vpx) $(wavpack) $(webp) $(x264) $(x265) $(xvid) $(zvbi) 
...
$(vidstab): extdir := vid.stab-1.1.0/build
...
$(gsm): makeopts := INSTALL_ROOT=$(instdir) GSM_INSTALL_INC=$(instdir)/include CCFLAGS="-c -O2 -DNeedFunctionPrototypes=1 -Wall -Wno-comment -fPIC"
...
$(mysofa): conf := cmake3 -DCMAKE_INSTALL_PREFIX:PATH=$(instdir) -DBUILD_TESTS=OFF -DCMAKE_INSTALL_LIBDIR=lib ..
...
$(rubberband): makeopts := JNI_TARGET=/dev/null
...
$(webp): conf := cmake3 -DCMAKE_INSTALL_PREFIX:PATH=$(instdir) -DLIBRARY_OUTPUT_PATH=$(instdir)/lib -DCMAKE_INSTALL_LIBDIR=$(instdir)/lib -DCMAKE_C_FLAGS=-fPIC ..
